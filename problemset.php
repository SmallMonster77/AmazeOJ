...
if(isset($_GET['typeid'])){
	$typeid=$_GET['typeid'];
	$filter_sql=" classify = $typeid";
}
if (isset($_SESSION['administrator'])){
	
	$sql="SELECT `problem_id`,`title`,`source`,`submit`,`accepted`,`classify` FROM `problem` WHERE $filter_sql ";
	
}
else{
	$now=strftime("%Y-%m-%d %H:%M",time());
	$sql="SELECT `problem_id`,`title`,`source`,`submit`,`accepted`,`classify` FROM `problem` ".
...
while ($row=mysql_fetch_object($result)){
	...
	if (isset($sub_arr[$row->problem_id])){
		if (isset($acc_arr[$row->problem_id])) 
			$temp="<span class='am-badge am-badge-success'>Y</span>";
		else 
			$temp= "<span class='am-badge am-badge-danger'>N</span>";
	}else{
		$temp= "<span></span>";
	}
	$view_problemset[$i][0]="<td>{$row->problem_id}{$temp}</td>";
	$view_problemset[$i][1]="<td><a href='problem.php?id={$row->problem_id}'>{$row->title}</a></td>";
	$view_problemset[$i][2]="<td>{$row->accepted}/{$row->submit}</td>";
	$view_problemset[$i][3]="<td>{$c[$row->classify-1]}</td>";	
	$i++;
}
